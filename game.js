...
function finishRun() {
  ...
  const gainedSupplies = Math.floor(score / 120) + stage * 3;
  ...
  startButton.textContent = "Start Next Mission";
  startMenu.style.display = "block";
  gameStarted = false;
  closeTutorial();
  stageBanner = `Run complete: +${gainedSupplies} supplies`;
...
function makeZombie(type) {
  if (type === "runner") {
    return {
      ...
      speed: randomRange(1.8, 2.5) + stage * 0.05,
      color: "#c5ff5a",
      reward: 16,
      shootCooldown: 999
    };
  ...
  if (type === "brute") {
    return {
      ...
      height: 30,
      hp: 5 + stage,
      speed: randomRange(0.7, 1) + stage * 0.02,
      color: "#84c750",
      reward: 30,
      shootCooldown: 999
    };
  ...
  if (type === "spitter") {
    return {
      ...
      width: 32,
      height: 24,
      hp: 2 + Math.floor(stage * 0.7),
      speed: randomRange(1.1, 1.5) + stage * 0.03,
      color: "#7af08c",
      reward: 22,
      shootCooldown: 80
    };
  ...
  return {
    ...
    height: 26,
    hp: 1 + Math.floor(stage / 4),
    speed: randomRange(0.9, 1.3) + stage * 0.04,
    color: "#9de96a",
    reward: 12,
    shootCooldown: 999
  };
...
function spawnBoss() {
  ...
  boss = {
    ...
    hp,
    maxHp: hp,
    speed: 2.2 + stage * 0.16,
    direction: 1,
    shootCooldown: 50,
    slamCooldown: 160
  };
  stageBanner = `Mutant Boss - Sector ${stage}`;
...
function applyExplosionDamage(explosion) {
  ...
  if (boss) {
    ...
    if (distance <= explosion.radius + 40) {
      ...
      if (boss.hp <= 0) {
        score += 500 + stage * 140;
        maybeDropPickup(boss.x + 60, boss.y + boss.height + 10, "upgrade");
        maybeDropPickup(boss.x + 130, boss.y + boss.height + 10, "life");
        advanceStage();
      }
    }
  }
...
function updateStageFlow() {
  ...
  if (!boss) {
    spawnTimer += 1;
    const spawnRate = Math.max(34, 72 - stage * 2);
...
function drawGameOver() {
  ...
  ctx.fillText("YOU WERE OVERRUN", GAME_WIDTH / 2, GAME_HEIGHT / 2 - 30);
  ctx.font = "22px Trebuchet MS";
  ctx.fillText(`Final Score: ${score}`, GAME_WIDTH / 2, GAME_HEIGHT / 2 + 10);
  ctx.fillText("Choose loadout and start next mission", GAME_WIDTH / 2, GAME_HEIGHT / 2 + 44);
  ctx.restore();
}
